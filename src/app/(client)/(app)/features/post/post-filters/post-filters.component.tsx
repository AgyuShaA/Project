...
export default function PostFilters({ search, sortOrder, setSearch, setSortOrder }: PostFiltersProps) {
    ...
    <div className='mb-6 flex flex-col sm:flex-row sm:items-center sm:gap-6'>
      ...
      <div className='flex flex-wrap gap-2'>
        {sortButtons.map(({ label, order }) => (
          <Button key={order} onClick={() => handleSort(order)} className={buttonClass(order)}>
            {label}
          </Button>
        ))}
      </div>
