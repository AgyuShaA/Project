...
export async function proxy(req: NextRequest) {
  ...
  const pathWithoutLocale = segments.length > 1 ? '/' + segments.slice(1).join('/') : '/' + segments[0] || '/'
  ...
  if (!session) {
    ...
    return NextResponse.redirect(new URL(`/${predefinedLocale}/login`, req.url), { headers: i18nRes.headers })
  }
